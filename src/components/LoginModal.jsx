...
export function LoginModal() {
  ...
  function show() { setOpenModal(true)}
  function hide() {reset(); clear(); setOpenModal(false)}
  
  return (
    <div>
      <Button size='sm' color='gray' onClick={show}>Login</Button>
        ...
        <Modal.Body>
          <form onSubmit={handleSubmit(onSubmit)} className='grid gap-2'>
            ...
            {error && <div className='text-red-600'>
              Failed to authenticate</div>}
            <div className="flex mt-3 items-center gap-2">
              <Button className='flex-1' type='submit' 
                isProcessing={isLoading} disabled={isLoading}>
                Submit
              </Button>
